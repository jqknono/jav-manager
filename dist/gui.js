...
function renderSearchPage(loc, data) {
    ...
    const torrentSection = torrents.length
        ? renderCard(loc.get("gui_section_torrents"), `<table>${tableHeader(["Title", "Size", ""])}<tbody>${torrentRows}</tbody></table>`)
        : "";
    return renderLayout(loc, loc.get("gui_nav_search"), `
      ${renderStatus(loc, data.status)}
      ${renderCard(loc.get("gui_nav_search"), `<form method="post" action="/search">
...
function renderSettingsPage(loc, context, status) {
    ...
    return renderLayout(loc, loc.get("gui_nav_settings"), `
      ...
      ${renderCard(loc.get("gui_nav_settings"), `<form method="post" action="/settings">
          <h3>Everything</h3>
...
function renderLayout(loc, title, content) {
    return `<!doctype html>

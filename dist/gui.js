...
const platformShell_1 = require("./utils/platformShell");
...
function startGuiServer(context, port = defaultPort, host = defaultHost) {
    ...
    app.post("/search", async (req, res) => {
        ...
        res.send(renderSearchPage(context.loc, externalLinks, {
            query,
            searchLocal,
            searchRemote,
            status,
    ...
    app.get("/detail", async (req, res) => {
        ...
        try {
            ...
            if (detail && !detail.javId) {
                detail.javId = javId || (0, torrentNameParser_1.normalizeJavId)(detail.title);
            }
            if (detail) {
                context.services.telemetryClient.tryReport(detail);
                if (context.services.cacheProvider && detail.torrents.length > 0) {
                    try {
                        await context.services.cacheProvider.save(detail);
                    }
                    catch { /* ignore */ }
                }
            }
        ...
        const torrents = detail
            ...
            : [];
        res.send(renderDetailPage(context.loc, externalLinks, {
            query,
            searchLocal,
            searchRemote,
            status,
    ...
    app.post("/api/test-all", async (req, res) => {
        try {
            ...
            const checkers = [
                new everything_1.EverythingHttpClient(everythingCfg),
                new qbittorrent_1.QBittorrentApiClient(qbCfg),
                new javdb_1.JavDbWebScraper(javDbCfg),
            ];
    ...
    app.post("/api/open-folder", async (req, res) => {
        ...
        try {
            await (0, platformShell_1.openContainingFolder)(filePath);
            res.json({ ok: true });
        }
        catch {
            res.json({ ok: false });
...
function buildCompletionStatus(loc, searchLocal, searchRemote) {
    if (searchLocal && searchRemote)
        return loc.get("gui_status_local_and_remote_complete");
    if (searchLocal)
        return loc.get("gui_status_local_search_complete");
    if (searchRemote)
        return loc.get("gui_status_remote_search_complete");
...
function renderSearchPage(loc, links, data) {
    ...
    const localSection = localFiles.length
        ? renderCard(loc.get("gui_section_local_files"), `<ul class="file-list">${localFiles
            .map((file) => `<li class="file-item">
    ...
    const candidateRows = candidates
        .map((item, idx) => {
        const javId = item.javId || (0, torrentNameParser_1.normalizeJavId)(item.title);
    ...
    const candidateSection = candidates.length
        ? renderCard(`${loc.get("gui_search_results")} ${candidateCount}`, `<div class="table-wrap"><table>${tableHeader([
            "ID",
            loc.get("gui_table_title"),
            loc.get("gui_table_date"),
            loc.get("gui_table_actors"),
            "",
        ])}<tbody>${candidateRows}</tbody></table></div>`, false)
        : "";
    return renderLayout(loc, loc.get("gui_nav_search"), `
      ${renderStatus(loc, data.status)}
      ${renderCard("", `<form method="post" action="/search" class="search-form" id="searchForm">
...
function renderDetailPage(loc, links, data) {
    ...
    if (detail) {
        ...
        const coverHtml = detail.coverUrl
            ...
            : "";
        infoSection = renderCard(`${escapeHtml(data.javId)} — ${escapeHtml(detail.title)}`, `<div class="detail-layout">
        ${coverHtml}
...
function formatLocalSearchError(loc, error) {
    const message = error instanceof Error ? error.message : "Unknown error";
...
function formatRemoteSearchError(loc, error) {
    const message = error instanceof Error ? error.message : "Unknown error";
    return loc.getFormat("gui_error_remote_search", message);
}
function renderSettingsPage(loc, links, context, status) {
    const cfg = context.config;
    const extLink = (url, label) => `<a class="card-ext-link" href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer" title="${escapeHtml(label)}" aria-label="${escapeHtml(label)}">↗</a>`;
    return renderLayout(loc, loc.get("gui_nav_settings"), `
      ...
      <form method="post" action="/settings" class="settings-grid">
        ${renderCard(`${loc.get("gui_settings_section_everything")} ${extLink(links.everythingUrl, loc.get("gui_nav_everything"))}`, `${inputField(loc.get("gui_field_base_url"), "everythingBaseUrl", cfg.everything.baseUrl)}
          ${inputField(loc.get("gui_field_username"), "everythingUserName", cfg.everything.userName ?? "")}
          ${inputField(loc.get("gui_field_password"), "everythingPassword", cfg.everything.password ?? "", "", "password")}`, false)}
        ${renderCard(`${loc.get("gui_settings_section_qbittorrent")} ${extLink(links.qBittorrentUrl, loc.get("gui_nav_qbittorrent"))}`, `${inputField(loc.get("gui_field_base_url"), "qbBaseUrl", cfg.qBittorrent.baseUrl)}
          ${inputField(loc.get("gui_field_username"), "qbUserName", cfg.qBittorrent.userName ?? "")}
          ${inputField(loc.get("gui_field_password"), "qbPassword", cfg.qBittorrent.password ?? "", "", "password")}`, false)}
        ${renderCard(`${loc.get("gui_settings_section_javdb")} ${extLink(links.javDbUrl, loc.get("gui_nav_javdb"))}`, `${inputField(loc.get("gui_field_base_url"), "javDbBaseUrl", cfg.javDb.baseUrl)}
          ${inputField(loc.get("gui_field_mirror_urls"), "javDbMirrorUrls", cfg.javDb.mirrorUrls.join(","))}`, false)}

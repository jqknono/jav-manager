...
function renderDownloadsPage(loc: LocalizationService, links: GuiExternalLinks, torrents: TorrentInfo[], status?: string): string {
  const rows = torrents
    .map(
      (torrent) => `<tr>
  ...
  const content = `
    ${statusHtml}
    ${renderCard(
      `${loc.get("gui_nav_downloads")} ${dlCount}`,
